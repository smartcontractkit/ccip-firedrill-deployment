...
var FiredrillOffRampDiscriminator = [8]byte{130, 119, 71, 128, 162, 166, 175, 201}

func (obj FiredrillOffRamp) MarshalWithEncoder(encoder *ag_binary.Encoder) (err error) {
	// Write account discriminator:
	err = encoder.WriteBytes(FiredrillOffRampDiscriminator[:], false)
	if err != nil {
		return err
	}
	// Serialize `Owner` param:
...
func (obj *FiredrillOffRamp) UnmarshalWithDecoder(decoder *ag_binary.Decoder) (err error) {
	// Read and check account discriminator:
	{
		discriminator, err := decoder.ReadTypeID()
		if err != nil {
		...
		if !discriminator.Equal(FiredrillOffRampDiscriminator[:]) {
			return fmt.Errorf(
				"wrong discriminator: wanted %s, got %s",
				"[130 119 71 128 162 166 175 201]",
				fmt.Sprint(discriminator[:]))
		}
	}
	// Deserialize `Owner`:
